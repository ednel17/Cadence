...
function productCard(p){
  ...
  return `
    <article class="pcard">
      <div class="pcover" data-action="view" data-id="${p.id}">
      ...
      <div class="pbody">
        ...
        <div class="pfoot">
          ${price}
          <div class="pbtns">
            <button class="pview" data-action="view" data-id="${p.id}">Details</button>
            <button class="padd" data-action="add" data-id="${p.id}">Add</button>
          </div>
        </div>
      </div>
    </article>`;
...
function bumpCount(){
  ...
  el.style.transform = 'scale(1.3)'; setTimeout(()=> el.style.transform = 'scale(1)', 160);
}
function renderCart(){
...
/* ============================================================ PRODUCT MODAL (with Preview Inside) */
function openModal(id){
  const p = byId(id); if(!p) return;
  const price = p.compareAt
    ? `<span class="mb-price"><s style="font-family:var(--font-ui);font-size:16px;color:var(--muted);font-weight:400;margin-right:8px">${money(p.compareAt)}</s>${money(p.price)}</span>`
    : `<span class="mb-price">${money(p.price)}</span>`;
  const fmtTag = p.type==='hybrid' ? 'Paper + digital' : p.type==='digital' ? 'Instant download' : p.type==='accessories' ? 'Add-on' : 'Ships free $50+';
  const mini = miniPreviews(id);
  $('#modalBody').innerHTML = `
    <div class="mb-cover" style="background:${GRAD[p.category]}">
      <div class="mbc-strip"><i></i><i></i><i></i><i></i><i></i><i></i></div>
      <div class="mbc-title"><span>Cadence</span><b>${p.name}</b></div>
    </div>
    <div class="mb-info">
      <div class="pcat">${p.category} · ${typeNote[p.type]}</div>
      <h3>${p.name}</h3>
      <div class="mb-rating"><span class="rs">★★★★★</span> ${p.rating} · ${p.reviews.toLocaleString()} reviews</div>
      <div class="mb-fmt"><span>${badgeText[p.type]}</span><span>Start any month</span><span>${fmtTag}</span></div>
      <div class="mb-tabs">
        <button class="mb-tab active" data-action="mb-tab" data-tab="details">Details</button>
        <button class="mb-tab" data-action="mb-tab" data-tab="preview">Preview inside</button>
      </div>
      <div class="mb-pane active" data-pane="details">
        <p class="mb-desc">${p.desc}</p>
        <div class="mb-sub">Why it works</div>
        <ul class="mb-benefits">${p.benefits.map(b=>`<li>${b}</li>`).join('')}</ul>
        <div class="mb-sub">What’s included</div>
        <ul class="mb-benefits">${p.incl.map(b=>`<li>${b}</li>`).join('')}</ul>
      </div>
      <div class="mb-pane" data-pane="preview">
        <div class="mb-mtabs">
          <button class="mb-mtab active" data-action="mb-mini-tab" data-mtab="m-month" type="button">Month</button>
          <button class="mb-mtab" data-action="mb-mini-tab" data-mtab="m-week" type="button">Week</button>
          <button class="mb-mtab" data-action="mb-mini-tab" data-mtab="m-addon" type="button">Add-on</button>
          <button class="mb-mtab" data-action="mb-mini-tab" data-mtab="m-comp" type="button">Digital companion</button>
        </div>
        <div class="mb-mpane active" data-mpane="m-month"><div class="mb-mini"><div class="pv" style="${mini.vars}">${mini.month}</div></div><p class="mb-mini-copy">${mini.copy.month}</p></div>
        <div class="mb-mpane" data-mpane="m-week"><div class="mb-mini"><div class="pv" style="${mini.vars}">${mini.week}</div></div><p class="mb-mini-copy">${mini.copy.week}</p></div>
        <div class="mb-mpane" data-mpane="m-addon"><div class="mb-mini"><div class="pv" style="${mini.vars}">${mini.addon}</div></div><p class="mb-mini-copy">${mini.copy.addon}</p></div>
        <div class="mb-mpane" data-mpane="m-comp"><div class="mb-mini mb-mini-dark"><div class="pv" style="${mini.vars}">${mini.companion}</div></div><p class="mb-mini-copy">${mini.copy.companion}</p></div>
      </div>
      <div class="mb-buy">
        ${price}
        <button class="padd" style="padding:13px 22px;font-size:15px" data-action="add" data-id="${p.id}">Add to cart</button>
        <button class="btn btn-line" data-action="pv-load" data-id="${p.id}">Customize this exact system →</button>
      </div>
    </div>`;
...
function renderMainPreview(){
  const pal = PALETTE[PV.palette], f = FONT[PV.font];
  const vars = `--pv-cover:${pal.cover};--pv-accent:${pal.accent};--pv-accent-2:${pal.accent2};--pv-font:${f.stack}`;
  const cls  = `pv ${pal.dark ? 'dark-cover':''} ${PV.font==='editorial' ? 'f-editorial':''}`;
  let html;
  if(PV.mode === 'compare' && PV.view !== 'cover'){
    html = renderComparisonPreview();
  } else if(PV.view === 'cover'){
    if(PV.format === 'digital'){
      html = `<div class="pv-frame tablet"><div class="pv-tablet"><div class="pv-screen"><div class="pv-statusbar"><span>9:41</span><b>Cadence</b><span class="dots"><b></b><b></b><b></b></span></div><div style="padding:14px;background:#fff">${coverHTML(false)}</div></div></div></div>`;
    } else {
      html = `<div class="pv-frame" style="display:flex;justify-content:center;width:100%">${coverHTML(false)}</div>`;
    }
  } else if(PV.format === 'hybrid'){
    html = renderHybridPreview();
  } else {
    html = frameHTML(PV.format, `<div class="pv-sheet" style="position:relative">${pageContent(PV.view)}</div>`);
  }
  $('#stStage').innerHTML = `<div class="${cls}" style="${vars}">${html}</div>`;
}
...
/* ---- "Customize this system" → preload the studio ---- */
function customizeFrom(id){
  const c = PRELOAD[id]; if(!c) return;
  const p = byId(id);
  Object.assign(PV, { format:'hybrid', view:'month', layout:'vertical', month:'January', palette:'Soft Minimal', font:'serif', name:'', addons:[], sample:false }, c);
  PV.title = p ? p.name : PV.title;
  syncAllControls();
  renderPreviewStudio();
  closeModal();
  $('#build').scrollIntoView({ behavior:'smooth' });
  setTimeout(()=> updatePreviewMessage(`Preloaded for ${p?p.name:'your pick'} — tweak anything you like.`), 350);
  toast('Preview Studio set up for ' + (p?p.name:'you'));
}
...
function gotoShop(){ const s = $('#shop'); if(s) s.scrollIntoView({ behavior:'smooth' }); }
...
/* ---------- assemble a page for a given config ---------- */
function pageInner(cfg){
  if(cfg.pageType==='cover'){
    const cov = ppCover(cfg);
    if(cfg.format==='digital') return `<div class="fr-tablet"><div class="scr"><div class="pp-toolbar"><span class="tdot"></span><span class="tt">Cadence</span><span class="tsp"></span><span class="tt">✎ ▢</span></div><div style="padding:16px;background:#fff">${cov}</div></div></div>`;
    return `<div style="display:flex;justify-content:center;width:100%">${cov}</div>`;
  }
    ...
    <article class="pcard">
      ...
      <div class="pbody">
        ...
        <div class="pfoot" style="display:block">
          <div style="margin-bottom:8px">${price}</div>
          <div class="pc-actions">
            <button class="pview" data-action="view-system" data-id="${p.id}">View system</button>
            <button class="pview" data-action="customize-system" data-id="${p.id}">Customize</button>
            <button class="padd" data-action="add" data-id="${p.id}">Add to cart</button>
          </div>
        </div>
      </div>
    </article>`;
...
/* OVERRIDE — "Customize" loads the product's template defaults into the studio */
function customizeFrom(id){
...
/* ============================================================================
   ...
   ============================================================================ */
let pvwState = { id:null, seq:[], idx:0, tab:'details' };
const REVIEW_BANK = [
...
function templateCfg(id, pageType){
  ...
  return { pageType, format:T.defaultFormat||'physical', layout:T.defaultLayout||'vertical', palette:T.defaultPalette||'Soft Minimal',
           font:T.defaultFont||'serif', month:T.defaultStartMonth||'January', title:p?p.name:'', name:'', addons:(T.includedAddons||[]).slice(), sample:true };
}
...
function reviewsHTML(p){
  return REVIEW_BANK.slice(0,3).map(r=>`<div class="pvw-review"><div class="rh"><span class="rn">${r[0]}</span><span class="rs">★★★★★</span></div><p>${r[1]}</p></div>`).join('')
    + `<p style="font-size:12px;color:var(--muted);margin-top:4px">${p.rating} average · ${p.reviews.toLocaleString()} verified reviews</p>`;
}
function pvwShell(p){
  const T = PRODUCT_TEMPLATES[p.name] || {};
  const price = p.compareAt
    ? `<span class="pvw-price"><s>${money(p.compareAt)}</s>${money(p.price)}</span>`
    : `<span class="pvw-price">${money(p.price)}</span>`;
  const fmtTag = p.type==='hybrid'?'Paper + digital':p.type==='digital'?'Instant download':p.type==='accessories'?'Add-on':'Ships free $50+';
  const inclPages = productSequence(p.id).map(pageName).filter((v,i,a)=>a.indexOf(v)===i);
  const inclList = inclPages.map(x=>`<li>${x}</li>`).join('') + (T.includedAddons||[]).map(x=>`<li>${x}</li>`).join('');
  const tab = pvwState.tab;
  return `
    <div class="pvw-top"><h3>${p.name}</h3>${price}</div>
    <div class="pvw-meta"><span style="color:var(--gold-soft)">★ ${p.rating}</span><span>${p.reviews.toLocaleString()} reviews</span><span>${badgeText[p.type]}</span><span>Starts ${T.defaultStartMonth||'any month'}</span><span>${fmtTag}</span></div>
    <div class="pvw-tabs">
      <button class="pvw-tab ${tab==='details'?'on':''}" data-action="vw-tab" data-tab="details" type="button">Details</button>
      <button class="pvw-tab ${tab==='view'?'on':''}" data-action="vw-tab" data-tab="view" type="button">View current system</button>
      <button class="pvw-tab ${tab==='incl'?'on':''}" data-action="vw-tab" data-tab="incl" type="button">What’s included</button>
      <button class="pvw-tab ${tab==='reviews'?'on':''}" data-action="vw-tab" data-tab="reviews" type="button">Reviews</button>
    </div>
    <div class="pvw-pane ${tab==='details'?'on':''}" data-pane="details">
      <p style="font-size:14.5px;color:var(--ink-soft);line-height:1.55">${p.desc}</p>
      <div class="mb-sub">Why it works</div>
      <ul class="mb-benefits">${p.benefits.map(b=>`<li>${b}</li>`).join('')}</ul>
      <p style="font-size:13px;color:var(--muted);margin-top:6px">Tip: open <b>View current system</b> to flip through the exact pages, or <b>Customize current system</b> to make it yours.</p>
    </div>
    <div class="pvw-pane ${tab==='view'?'on':''}" data-pane="view">
      <div class="pvw-stagewrap"><div id="pvwStage" style="width:100%"></div></div>
      <div class="pvw-nav">
        <button class="pvw-arrow" id="pvwPrev" data-action="vw-prev" type="button" aria-label="Previous page">‹</button>
        <div class="pvw-label" id="pvwLabel"></div>
        <button class="pvw-arrow" id="pvwNext" data-action="vw-next" type="button" aria-label="Next page">›</button>
      </div>
      <button class="view-larger" data-action="fs-viewer" type="button">⤢ View full page</button>
      <div class="pvw-thumbs" id="pvwThumbs"></div>
    </div>
    <div class="pvw-pane ${tab==='incl'?'on':''}" data-pane="incl">
      <div class="pvw-incl"><b>Included in this system</b><ul>${inclList}</ul></div>
      <div class="pvw-incl" style="margin-top:10px"><b>In the box</b><ul>${p.incl.map(x=>`<li>${x}</li>`).join('')}</ul></div>
    </div>
    <div class="pvw-pane ${tab==='reviews'?'on':''}" data-pane="reviews">${reviewsHTML(p)}</div>
    <div class="pvw-cta">
      ${price}
      <button class="btn btn-line" data-action="customize-current" type="button">Customize current system</button>
      <button class="btn btn-primary" data-action="add-system" type="button">Add current system to cart</button>
    </div>`;
}
function pvwRenderStage(){
  ...
  const th = $('#pvwThumbs');
  if(th) th.innerHTML = seq.map((p,i)=>`<div class="pvw-thumb ${i===idx?'on':''}" data-action="vw-thumb" data-i="${i}" role="button" tabindex="0"><div class="mini">${viewerPageHTML(id,p)}</div><span>${pageName(p)}</span></div>`).join('');
}
...
function pvwTab(name){
  pvwState.tab = name;
  $$('#modalBody .pvw-tab').forEach(x => x.classList.toggle('on', x.dataset.tab === name));
  $$('#modalBody .pvw-pane').forEach(x => x.classList.toggle('on', x.dataset.pane === name));
  if(name==='view' && $('#pvwStage') && !$('#pvwStage').innerHTML) pvwRenderStage();
}

/* OVERRIDE — modal is now a full product viewer */
function openModal(id, tab){
  const p = byId(id); if(!p) return;
  pvwState = { id, tab: tab || 'details', seq: productSequence(id), idx:0 };
  $('#modalBody').innerHTML = pvwShell(p);
  if(pvwState.tab === 'view') pvwRenderStage();
  $('#modal').classList.add('open'); $('#modal').setAttribute('aria-hidden','false');
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.addEventListener('click', e => {
    const t = e.target.closest('[data-action]'); if(!t) return;
    const a = t.dataset.action;
    if(a === 'view-system'){ openModal(+t.dataset.id, 'view'); }
...
function currentRoute(){
  ...
  return (m && ROUTE_TITLES[m[1]]) ? m[1] : 'home';
}
function setActiveNav(r){ $$('[data-route]').forEach(a => a.classList.toggle('rt-active', a.dataset.route === r)); }
function route(opts){
  const r = currentRoute();
  $$('.page-view').forEach(p => p.classList.toggle('active', p.dataset.page === r));
  setActiveNav(r);
  document.title = ROUTE_TITLES[r] || 'Cadence';
  const mm = $('#mobileMenu'); if(mm) mm.classList.remove('open');
...
/* OVERRIDE — update every cart badge (nav + bottom nav) */
function bumpCount(){
  const n = state.cart.reduce((s,i)=> s + i.qty, 0);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* bottom-nav cart */
  const bnCart = $('#bnCart'); if(bnCart) bnCart.addEventListener('click', openCart);
