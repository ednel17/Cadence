...
function renderSummary(){
  ...
  const _sp = $('#ssbPrice'); if(_sp) _sp.textContent = money(PV.price);   /* keep the persistent buy-bar price in sync */
}
...
function setupQuiz(){
  if(!document.getElementById('quizSubmit')) return;   /* dark quiz removed from studio — no-op (inline guided card is used instead) */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.addEventListener('click', e => {
    ...
    if(link && link.dataset.scroll){
      ...
      setTimeout(()=>{
        const el = $('#'+sc); if(el) el.scrollIntoView({ behavior:'smooth', block:'start' });
        if(link.dataset.guide){                                  /* "Build my system" → open the inline guided card */
          const g = $('#stGuided'); const hc = $('#helpChoose');
          if(g) g.hidden = false;
          if(hc) setTimeout(()=> hc.scrollIntoView({ behavior:'smooth', block:'center' }), 380);
        }
      }, 140);
    }
  });
